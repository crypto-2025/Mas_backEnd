...
class userController {
    ...
    async login(req, res, next) {
        ...
        try {
            ...
            if (error.isJoi) {
                return res.status(400).json(new response({}, error.details[0].message));
            }
    ...
    async allUserList(req, res, next) {
        ...
        try {
            // Use validate method on the schema object
            ...
            let usersWithEarnings = result.docs.map((user) => {
                ...
                if (index !== -1) {
                    const { masBalance, fdusdBalance, usdtBalance, referralBalance } = earnings[index];
                    return {
                        ...user.toObject(),
                        masBalance,
                        fdusdBalance,
    ...
    async  getUser(req, res, next) {
        ...
        try {
            const { userName } = await validationSchema.validateAsync(req.params);
    
            const userResult = await userAllDetailsByUserName(userName);
            if (!userResult) {
                return apiError.notFound(responseMessage.USER_NOT_FOUND);
            }
  ...
  async order(req, res, next) {
    ...
    async profileFollowersList(req, res, next) {
        ...
        try {
            ...
            // Assuming followersList is a function that accepts user ID and validation data
            let followersListResult = await followersList(userResult.followers, value);
    ...
    async  profileFollowingList(req, res, next) {
        ...
        const validationSchema = Joi.object({
            page: Joi.number().optional().default(1), // Add a default value for pagination
            limit: Joi.number().optional().default(10), // Add a default value
        });
    
        try {
            // Validate the query parameters using validateAsync
            const validateBody = await validationSchema.validateAsync(req.query);
            ...
            // Fetch the following list based on user's following data
            const followingListResult = await followingList(userResult.following, validateBody);
    
            // Respond with the data
            return res.json(new response(followingListResult, responseMessage.DATA_FOUND));
        } catch (error) {
    ...
    async  mySubscriptions(req, res, next) {
        // Define the validation schema
        const validationSchema = Joi.object({
            limit: Joi.number().optional().default(10), // Default limit to 10
            page: Joi.number().optional().default(1),  // Default page to 1
        });
        ...
        try {
            ...
            // Fetch subscription data with pagination
            const result = await subscriptionWithPaginate(validateBody, userResult._id);
    ...
    async  mypurchases(req, res, next) {
        ...
        const validationSchema = Joi.object({
            limit: Joi.number().optional().default(10), // Add a default value
            page: Joi.number().optional().default(1),  // Add a default value
        });
        ...
        try {
            ...
            // Fetch the purchases with pagination
            const result = await purchasesWithPaginate(validateBody, userResult._id);
    ...
    async shareWithAudience(req, res, next) {
        // Define validation schema using Joi for incoming request body
        const validationSchema = Joi.object({
            title: Joi.string().required(),   // Title is required and must be a string
            details: Joi.string().required(), // Details are required and must be a string
            mediaUrl: Joi.string().optional(), // Media URL is optional
            nftIds: Joi.array().optional(),   // NFT IDs should be an array (optional)
            postType: Joi.string().optional(), // Post type is optional and should be a string
        });
    
        try {
            // Parse the nftIds if it is a stringified array (string of JSON array format)
            if (typeof req.body.nftIds === 'string') {
                req.body.nftIds = JSON.parse(req.body.nftIds);
            }
            ...
            // Destructure validated values from Joi validation
            const validatedBody = value;
    
            // Fetch user based on userId from the request (authentication assumed)
            let userResult = await findUser({ _id: req.userId });
            if (!userResult) {
                return apiError.notFound(responseMessage.USER_NOT_FOUND);
            }
    
            // Get the list of NFT subscribers by filtering the relevant NFTs
            let nftResult = await nftSubscriberList({
                _id: { $in: validatedBody.nftIds },
                status: { $ne: status.DELETE },
            });
    
            // Extract unique users from the subscribers of NFTs
            let users = nftResult.flatMap(nft => nft.subscribers).filter(Boolean); // Flatten and filter empty values
            users = Array.from(new Set(users)); // Remove duplicates by converting to a Set and back to an array
    
            // Handle media URL if present in the request, and clean up uploaded files
            validatedBody.mediaUrl = await commonFunction.getImageUrl(req.files);
            await deleteFile(req.files[0].path); // Delete file after processing
    
            // Attach additional properties to validated body for further processing
            validatedBody.userId = userResult._id; // Set the userId
            validatedBody.users = users;          // Attach unique users list
            validatedBody.nftId = validatedBody.nftIds; // Set nftId from nftIds
    
            // Create the audience post with the validated and enriched data
            const result = await createAudience(validatedBody);
    
            // Update the NFT bundle status to indicate sharing
            await multiUpdateBundle(
                { _id: { $in: validatedBody.nftIds } },
                { $set: { isShared: true } }
            );
            ...
            for (let userId of users) {
                notificationPayload.userId = userId; // Set individual user ID for notification
                await createNotification(notificationPayload); // Create a notification for the user
            }
    
            // Respond with a success message containing the result
            return res.json(new response(result, responseMessage.SHARED_AUDIENCE));
    
        } catch (error) {
            // Handle unexpected errors with a generic error response
            console.error('Error in shareWithAudience:', error);
    ...
    async  editAudience(req, res, next) {
        ...
        try {
            const { id, postType, details } = await validationSchema.validateAsync(req.body);
    ...
    async  likeDislikeNft(req, res, next) {
        ...
        let updated;
        try {
            const { nftId } = await validationSchema.validateAsync(req.params);
    ...
    async  likeDislikeFeed(req, res, next) {
        ...
        let updated;
        try {
            const { feedId } = await validationSchema.validateAsync(req.params);
            ...
            const feedCheck = await findAudience({
                _id: feedId,
                status: { $ne: status.DELETE },
            });
            if (!feedCheck) {
    ...
    async bundleContentList(req, res, next) {
        try {
            ...
            // Validate the request query using the schema
            const { error, value } = validationSchema.validate(req.query);
    
            if (error) {
                // If validation fails, you can throw an error with a message
                throw new Error('Invalid query parameters');
            }
    
            const validatedBody = value; // Use the validated values
    ...
    async  listBanner(req, res, next) {
        ...
        try {
            ...
            // Query the bannerModel with validated parameters and status
            const result = await bannerModel.paginate(
                { status: 'ACTIVE' },
                { page: validatedBody.page, limit: validatedBody.limit }
            );
    ...
    async donationTransactionlist(req, res, next) {
        ...
        try {
            ...
            // Use validated value instead of the raw req.query
            var result = await depositListWithPagination(
                userResult._id,
                value
            );
    ...
    async  getMyFeed(req, res, next) {
        ...
        try {
            // Validate query parameters using validateAsync
            const validateBody = await validationSchema.validateAsync(req.query);
            ...
            // Prepare the query for fetching feed
            const query = { users: { $in: [userResult._id] } };
    ...
    async transactionList(req, res, next) {
        ...
        try {
            ...
            data = await depositListWithPopulate(userResult._id, value); // Use the validated value
            return res.json(new response(data, responseMessage.DETAILS_FETCHED));
        } catch (error) {
            return next(error);
        }
