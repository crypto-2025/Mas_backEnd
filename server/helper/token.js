...
module.exports = class tokenERC {
  ...
  async trackDeposits(addrPool, callBack) {
    try {
      const tokenContract = new this.web3Websocket.eth.Contract(contractABI, this.address);
  
      const currentBlockNumber = await this.web3.eth.getBlockNumber();
      const chunkSize = 5; // Block range supported by your plan
      const fromBlock = Math.max(Number(currentBlockNumber) - 2000, 0);
  
      console.log("Start tracking from block number:", fromBlock);
  
      for (let start = fromBlock; start <= currentBlockNumber; start += chunkSize) {
        const end = Math.min(start + chunkSize - 1, currentBlockNumber);
  
        const options = {
          filter: {
            to: addrPool.map((addr) => addr.toLowerCase()),
          },
          fromBlock: start,
          toBlock: end,
        };
        ...
        try {
          const events = await tokenContract.getPastEvents("Transfer", options);
          for (const event of events) {
            const value = this.web3.utils.fromWei(event.returnValues.value, "ether");
            const hash = event.transactionHash;
            const from = event.returnValues.from.toLowerCase();
            const to = event.returnValues.to.toLowerCase();
  
            if (addrPool.includes(to)) {
              callBack(hash, this.coin, value, from, to);
            }
          }
        } catch (err) {
          console.error(`Error fetching logs for blocks ${start}-${end}:`, err.message);
        }
      }
    } catch (err) {
      console.error("Error initializing trackDeposits:", err.message);
    }
  }
